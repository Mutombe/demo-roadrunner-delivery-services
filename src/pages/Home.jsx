...
import {
  ...
  CaretLeft,
  CaretRight,
  Briefcase, Buildings, Heart, Lightbulb, Rocket,
  CheckCircle,
...
/* ================================================================
   UTILITY -- Icon map
   ================================================================ */
const iconMap = { Briefcase, Buildings, Heart, Lightbulb, Rocket, Star };
    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-slate-950">
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        ...
        <div className="overflow-hidden">
          {['SPEED', 'RELIABILITY', 'DELIVERED.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1
                className={`font-heading leading-[0.92] tracking-tight ${line === 'RELIABILITY' ? 'bg-gradient-to-r from-emerald-500 via-emerald-400 to-emerald-500 bg-clip-text text-transparent italic' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'RELIABILITY' ? 700 : 300 }}
              >
                {line}
              </h1>
            </motion.div>
      ...
      <div className="hidden lg:flex absolute right-8 bottom-12 z-20">
        <span className="text-white/10 text-[10px] uppercase tracking-[0.4em]" style={{ writingMode: 'vertical-rl', fontFamily: 'var(--font-sans)' }}>
          Roadrunner Delivery Services &mdash; 55 five Ave
        </span>
      </div>
    </section>
  );
...
function MarqueeTicker() {
  const items = ['COURIER SERVICES', 'SAME-DAY DELIVERY', 'NATIONWIDE FREIGHT', 'LAST-MILE LOGISTICS', 'WAREHOUSING', 'EXPRESS PARCELS'];
...
function WhyChooseUs() {
  ...
  const points = [
    { title: 'Real-Time Tracking', desc: 'Every shipment tracked from pickup to delivery with GPS precision and instant notifications.' },
    { title: 'On-Time Guarantee', desc: 'We stake our reputation on punctuality. Late deliveries are on us -- no questions asked.' },
    { title: 'Nationwide Network', desc: 'From Harare to Bulawayo, Mutare to Victoria Falls -- we cover every corner of Zimbabwe.' },
    { title: 'Dedicated Support', desc: 'Your own account manager plus 24/7 customer support for urgent shipments.' },
  ];
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          ...
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            YOUR PACKAGE<br /><span className="text-emerald-500">OUR PRIORITY</span>
          </h2>
